...
Automated Pre-Completion Checklist Gate verification script for sdd-agent.
...
Usage:
    python scripts/verify_release_gate.py [--root <project_root>]
"""
...
def check_gate(root_dir, backend_prefix="backend/", version_json_path="version.json", backend_pkg_path="backend/package.json", changelog_path="CHANGELOG.md"):
    print("=" * 60)
    print("🔒 RUNNING PRE-COMPLETION CHECKLIST GATE VERIFICATION (sdd-agent)")
    print("=" * 60)
    ...
    # Exclude package files from triggering backend check if they are the only ones modified
    ...
    # Check git status for version.json modification
...
def main():
    parser = argparse.ArgumentParser(description="Pre-Completion Checklist Gate Verifier for sdd-agent")
